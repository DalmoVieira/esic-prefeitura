...
import { Plus, Search, FileText, Clock, CheckCircle, Gavel } from 'lucide-react';
...
const Dashboard: React.FC = () => {
  ...
  const [requests] = useState([
    {
      ...
      description: 'Solicitação de cópia do contrato de pavimentação da Rua das Flores.',
      status: 'RESPONDED',
      date: '2026-03-15',
      deadline: '2026-04-04'
    },
    {
      ...
      description: 'Informações sobre a lotação de servidores na Secretaria de Saúde.',
      status: 'OPEN',
      date: '2026-03-20',
      deadline: '2026-04-09'
    }
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'OPEN':
        return <span style={{ backgroundColor: '#fff3cd', color: '#856404', padding: '0.25rem 0.75rem', borderRadius: '1rem', fontSize: '0.85rem', fontWeight: 'bold' }}>Aberto</span>;
      case 'RESPONDED':
        return <span style={{ backgroundColor: '#d4edda', color: '#155724', padding: '0.25rem 0.75rem', borderRadius: '1rem', fontSize: '0.85rem', fontWeight: 'bold' }}>Respondido</span>;
      default:
        return <span style={{ backgroundColor: '#e2e3e5', color: '#383d41', padding: '0.25rem 0.75rem', borderRadius: '1rem', fontSize: '0.85rem', fontWeight: 'bold' }}>{status}</span>;
    }
  };
  ...
  return (
    <div className="container">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div>
          <h1 style={{ marginBottom: '0.5rem' }}>Painel do Cidadão</h1>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead style={{ backgroundColor: '#f9f9f9' }}>
              <tr>
                ...
                <th style={{ textAlign: 'left', padding: '1rem 1.5rem', fontSize: '0.85rem', textTransform: 'uppercase', color: 'var(--text-muted)' }}>Data</th>
            ...
            <tbody>
              {requests.map(req => (
                <tr key={req.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                  ...
                  <td style={{ padding: '1rem 1.5rem' }}>{new Date(req.date).toLocaleDateString('pt-BR')}</td>
