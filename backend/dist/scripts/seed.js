...
async function main() {
    ...
    const senha = await bcryptjs_1.default.hash('Mudar@123', 10);
    // ── 1. DEPARTAMENTOS ─────────────────────────────────────────────────────────
    const deptData = [
    ...
    for (const d of deptData) {
        const dept = await prisma_1.prisma.department.upsert({
            where: { name: d.name },
    ...
    const tecSaude = await prisma_1.prisma.user.upsert({
    ...
    const tecEducacao = await prisma_1.prisma.user.upsert({
    ...
    const autoridade = await prisma_1.prisma.user.upsert({
    ...
    const controle = await prisma_1.prisma.user.upsert({
    ...
    for (const req of requestsData) {
        const r = await prisma_1.prisma.request.upsert({
    ...
    for (const m of movementsData) {
        if (!m.requestId)
            continue;
        await prisma_1.prisma.movement.create({ data: { ...m, requestId: m.requestId } });
    }
    ...
    for (const a of appealsData) {
        if (!a.requestId)
            continue;
        await prisma_1.prisma.appeal.create({ data: { ...a, requestId: a.requestId } });
    }
    console.log('✅ Recursos criados.');
    // ── 6. CONFIGURAÇÃO MUNICIPAL ─────────────────────────────────────────────────
    await prisma_1.prisma.municipalConfig.upsert({
        where: { id: '1' },
        update: {},
        create: {
            id: '1',
